...
sys.path.append(os.path.join(cwd, 'Process'))

# print(cwd)

from Process.process import *
from Process.perturbation import *
# print(cwd)
import torch as th
...
def train_GCN(treeDic, x_test, x_train,TDdroprate,BUdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter):
    ...
    for epoch in range(n_epochs):
        ...
        # can add pertubation here ? # CN, CD, CE, PR, PU, PI
        # train_temp = []
        # for i in range(len(traindata_list)) :
        #     n = np.random.choice(np.arange(6), 1)
        #     perb = np.random.choice(('CN', 'CD', 'CE', 'PR', 'PU', 'PI'), n + 1)
        #     temp = traindata_list[i]
        #     for p in perb :
        #         temp = pertubation(temp, p)
        #     train_temp.append(temp)
                
        # train_loader = DataLoader(train_temp, batch_size=batchsize, shuffle=True, num_workers=5)
        ...
        for Batch_data in tqdm_train_loader:
            ...
            for i in range(len(Batch_data) - 1) :
                ...
                perb = np.random.choice(('CN', 'CD', 'CE', 'PR', 'PU', 'PI'), n + 1)
                for p in perb :
                    t = pertubation(t, p, 0.05)
                    t1 = pertubation(t1, p, 0.05)
                temp.append(t)
                temp1.append(t1)
            temp = tqdm(DataLoader(temp, batch_size = len(temp), shuffle = False))
            temp1 = tqdm(DataLoader(temp1, batch_size = len(temp), shuffle = False))
            
            for a, b in zip(temp, temp1):
                ma = model(a)
                mb = model(b)
                for i, j, k in zip(ma, mb, out_labels) :
                    pos = torch.inner(i, k)
                    pos = torch.exp(pos / 0.5)
                    neg = torch.inner(j, k)
                    neg = torch.exp(neg / 0.5)
                    l1 += -torch.log((pos / (pos + neg)) / 3)
